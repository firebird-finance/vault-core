...
abstract contract VaultBase is ERC20UpgradeSafe, IVault {
    ...
    modifier checkContract(address _account) {
        if (!acceptContractDepositor && !whitelistedContract[_account] && !whitelistedDepositor[_account]
                    && _account != vaultMaster.bank(address(this)) && _account != vaultMaster.bankMaster()) {
    ...
    function setWhitelistedDepositor(address _depositor) external {
        require(msg.sender == governance, "!governance");
        whitelistedDepositor[_depositor] = true;
    }

    function unwhitelistedDepositor(address _depositor) external {
        require(msg.sender == governance, "!governance");
        whitelistedDepositor[_depositor] = false;
    }
